...
extern void gtthread_app_init(unsigned int lb_flag, unsigned int prio_flag)
{
...
extern void gtthread_app_exit()
{
	...
	while(!(k_ctx->kthread_flags & KTHREAD_DONE))
	{
		...
		{
			/* siglongjmp to this point is done when there
			 * are no more uthreFads to schedule.*/
			/* XXX: gtthread app cleanup has to be done. */
			continue;
		}
		if (!k_ctx->prio_flag)
			credit_scheduler(&credit_find_best_uthread);
			// fprintf(stderr, "Inside gtthread_app_exit, this is done\n");
		else
			uthread_schedule(&sched_find_best_uthread);
	}
